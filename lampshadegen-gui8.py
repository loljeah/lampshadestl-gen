...
class LampshadeStyle(Enum):
    ...
    # --- Topology & Higher-Dimensional Projections ---
    KLEIN_BOTTLE_IMMERSION = "klein_bottle_immersion"
    HOPF_FIBRATION_BANDS = "hopf_fibration_bands"
    MOEBIUS_STRIP_WRAP = "moebius_strip_wrap"
    HYPER_CUBE_SHADOW = "hyper_cube_shadow"
    CALABI_YAU_PROJECTION = "calabi_yau_projection"
    LISSAJOUS_CAGE = "lissajous_cage"
    SEIFERT_SURFACE_KNOT = "seifert_surface_knot"
    STEREOGRAPHIC_TILING = "stereographic_tiling"
    PENROSE_QUASICRYSTAL = "penrose_quasicrystal"
    WEIERSTRASS_MONSTER = "weierstrass_monster"
...
MODIFIER_TOGGLES = {
    ...
    "enable_helical_ribbing": True,
    "enable_breathing_pulse": True,
}

# --- Surface Modifier Toggles (post-process displacement) ---
SURFACE_MODIFIER_TOGGLES = {
    "enable_smod_quantize": True,
    "enable_smod_foldback": True,
    "enable_smod_domain_warp": True,
    "enable_smod_fbm_overlay": True,
    "enable_smod_signed_power": True,
    "enable_smod_moire": True,
    "enable_smod_reaction_warp": True,
    "enable_smod_kaleidoscope": True,
}
...
class LampshadeParameters:
    ...
    # --- Topology & Higher-Dimensional Style Parameters ---
    klein_twist_rate: float = field(default_factory=lambda: random.uniform(0.5, 3.0))
    klein_amplitude: float = field(default_factory=lambda: random.uniform(2, 12))
    hopf_fiber_count: int = field(default_factory=lambda: random.randint(3, 12))
    hopf_twist_rate: float = field(default_factory=lambda: random.uniform(1.0, 5.0))
    hopf_amplitude: float = field(default_factory=lambda: random.uniform(2, 10))
    moebius_num_twists: int = field(default_factory=lambda: random.randint(1, 5))
    moebius_freq: int = field(default_factory=lambda: random.randint(2, 8))
    moebius_amplitude: float = field(default_factory=lambda: random.uniform(3, 10))
    hyper_rotation_speed: float = field(default_factory=lambda: random.uniform(0.5, 4.0))
    hyper_amplitude: float = field(default_factory=lambda: random.uniform(3, 12))
    hyper_dimension_fold: int = field(default_factory=lambda: random.randint(2, 6))
    calabi_exponent: int = field(default_factory=lambda: random.randint(2, 6))
    calabi_phase_drift: float = field(default_factory=lambda: random.uniform(0, 2 * np.pi))
    calabi_amplitude: float = field(default_factory=lambda: random.uniform(3, 15))
    liss_freq_a: int = field(default_factory=lambda: random.randint(1, 7))
    liss_freq_b: int = field(default_factory=lambda: random.randint(1, 7))
    liss_num_curves: int = field(default_factory=lambda: random.randint(3, 12))
    liss_amplitude: float = field(default_factory=lambda: random.uniform(2, 10))
    liss_width: float = field(default_factory=lambda: random.uniform(0.1, 0.5))
    seifert_p: int = field(default_factory=lambda: random.randint(2, 5))
    seifert_q: int = field(default_factory=lambda: random.randint(3, 7))
    seifert_amplitude: float = field(default_factory=lambda: random.uniform(3, 12))
    stereo_tile_p: int = field(default_factory=lambda: random.randint(3, 8))
    stereo_tile_q: int = field(default_factory=lambda: random.randint(3, 8))
    stereo_compression: float = field(default_factory=lambda: random.uniform(1.0, 4.0))
    stereo_amplitude: float = field(default_factory=lambda: random.uniform(2, 10))
    penrose_frequency: float = field(default_factory=lambda: random.uniform(3, 15))
    penrose_sharpness: float = field(default_factory=lambda: random.uniform(1.0, 4.0))
    penrose_amplitude: float = field(default_factory=lambda: random.uniform(2, 10))
    weier_b: float = field(default_factory=lambda: random.uniform(2.0, 7.0))
    weier_s: float = field(default_factory=lambda: random.uniform(0.2, 0.8))
    weier_num_terms: int = field(default_factory=lambda: random.randint(3, 10))
    weier_freq_t: int = field(default_factory=lambda: random.randint(2, 8))
    weier_freq_z: int = field(default_factory=lambda: random.randint(2, 8))
    weier_amplitude: float = field(default_factory=lambda: random.uniform(1, 8))
    ...
    # --- Surface Modifier Parameters (post-process displacement) ---
    enable_smod_quantize: bool = field(default_factory=lambda: False)
    smod_quantize_levels: int = field(default_factory=lambda: random.randint(3, 20))

    enable_smod_foldback: bool = field(default_factory=lambda: False)
    smod_fold_threshold: float = field(default_factory=lambda: random.uniform(0.5, 5.0))
    smod_fold_gain: float = field(default_factory=lambda: random.uniform(0.5, 2.0))

    enable_smod_domain_warp: bool = field(default_factory=lambda: False)
    smod_warp_amplitude: float = field(default_factory=lambda: random.uniform(0.1, 1.5))
    smod_warp_freq_t: int = field(default_factory=lambda: random.randint(1, 6))
    smod_warp_freq_z: int = field(default_factory=lambda: random.randint(1, 6))
    smod_warp_iterations: int = field(default_factory=lambda: random.randint(1, 3))

    enable_smod_fbm_overlay: bool = field(default_factory=lambda: False)
    smod_fbm_octaves: int = field(default_factory=lambda: random.randint(2, 8))
    smod_fbm_lacunarity: float = field(default_factory=lambda: random.uniform(1.5, 3.0))
    smod_fbm_gain: float = field(default_factory=lambda: random.uniform(0.3, 0.7))
    smod_fbm_amplitude: float = field(default_factory=lambda: random.uniform(1, 8))
    smod_fbm_base_freq: float = field(default_factory=lambda: random.uniform(2, 10))

    enable_smod_signed_power: bool = field(default_factory=lambda: False)
    smod_power_exponent: float = field(default_factory=lambda: random.uniform(0.2, 5.0))

    enable_smod_moire: bool = field(default_factory=lambda: False)
    smod_moire_freq1: int = field(default_factory=lambda: random.randint(5, 20))
    smod_moire_freq2: int = field(default_factory=lambda: random.randint(6, 21))
    smod_moire_z_freq1: int = field(default_factory=lambda: random.randint(3, 10))
    smod_moire_z_freq2: int = field(default_factory=lambda: random.randint(4, 11))
    smod_moire_amplitude: float = field(default_factory=lambda: random.uniform(1, 8))
    smod_moire_phase: float = field(default_factory=lambda: random.uniform(0, 2 * np.pi))

    enable_smod_reaction_warp: bool = field(default_factory=lambda: False)
    smod_rd_freq_u: int = field(default_factory=lambda: random.randint(3, 12))
    smod_rd_freq_v: int = field(default_factory=lambda: random.randint(3, 12))
    smod_rd_sharpness: float = field(default_factory=lambda: random.uniform(1.0, 5.0))
    smod_rd_amplitude: float = field(default_factory=lambda: random.uniform(1, 8))

    enable_smod_kaleidoscope: bool = field(default_factory=lambda: False)
    smod_kaleidoscope_sectors: int = field(default_factory=lambda: random.randint(3, 12))
    smod_kaleidoscope_rotation: float = field(default_factory=lambda: random.uniform(0, 2 * np.pi))
    ...
    enable_helical_ribbing: bool = field(default_factory=lambda: random.choice([True, False]) if MODIFIER_TOGGLES.get("enable_helical_ribbing") else False)
    helix_count: int = field(default_factory=lambda: random.randint(2, 8))
    helix_pitch: float = field(default_factory=lambda: random.uniform(0.5, 4.0))
    helix_rib_height: float = field(default_factory=lambda: random.uniform(2, 10))
    helix_rib_sharpness: float = field(default_factory=lambda: random.uniform(1.0, 4.0))

    enable_breathing_pulse: bool = field(default_factory=lambda: random.choice([True, False]) if MODIFIER_TOGGLES.get("enable_breathing_pulse") else False)
    breath_base_freq: int = field(default_factory=lambda: random.randint(1, 5))
    breath_chirp_rate: float = field(default_factory=lambda: random.uniform(0.5, 3.0))
    breath_amplitude: float = field(default_factory=lambda: random.uniform(3, 15))
    breath_envelope_power: float = field(default_factory=lambda: random.uniform(0.5, 3.0))
    ...
    surface_modifier_chain: List[str] = field(default_factory=list, init=False)
    ...
    def build_modifier_chain(self):
        ...
        self.modifier_chain = [mod for mod, is_enabled in self.__dict__.items()
                               if mod.startswith("enable_") and is_enabled
                               and not mod.startswith("enable_smod_")
                               and mod != "enable_blending"]
        random.shuffle(self.modifier_chain)
        # Surface modifiers have a fixed order (see _apply_surface_modifiers)
        self.surface_modifier_chain = [mod for mod, is_enabled in self.__dict__.items()
                                       if mod.startswith("enable_smod_") and is_enabled]
    ...
    def get_config_string(self) -> str:
        ...
        config_lines.append("\n--- Surface Modifiers (Fixed Order) ---")
        if self.surface_modifier_chain:
            for mod in self.surface_modifier_chain:
                config_lines.append(f"  - {mod[13:].replace('_', ' ').title()}")
        else:
            config_lines.append("  None")
...
class LampshadeGenerator:
    ...
    def _get_base_radius_at_z(self, z_norm: float) -> float:
        ...
        for mod in p.modifier_chain:
            ...
            elif mod == "enable_breathing_pulse":
                chirp_freq = p.breath_base_freq * (1.0 + p.breath_chirp_rate * z_norm)
                envelope = 1.0 - abs(2.0 * z_norm - 1.0) ** p.breath_envelope_power
                base_radius += envelope * p.breath_amplitude * np.sin(z_norm * 2.0 * np.pi * chirp_freq)
    ...
    def _calculate_style_effect(self, style, r, theta, z_norm, x, y, z, intensity):
        ...
        # --- Topology & Higher-Dimensional Projections ---
        elif style == LampshadeStyle.KLEIN_BOTTLE_IMMERSION:
            u = theta
            v = z_norm * 2.0 * np.pi * p.klein_twist_rate
            displacement = p.klein_amplitude * (np.sin(v) * np.cos(u / 2.0) + np.cos(v) * np.sin(u / 2.0) * np.sin(u))

        elif style == LampshadeStyle.HOPF_FIBRATION_BANDS:
            golden = (1.0 + np.sqrt(5.0)) / 2.0
            fiber_phase = theta * p.hopf_fiber_count + z_norm * p.hopf_twist_rate * 2.0 * np.pi
            displacement = p.hopf_amplitude * np.sin(fiber_phase) * np.cos(fiber_phase * golden)

        elif style == LampshadeStyle.MOEBIUS_STRIP_WRAP:
            moebius_t = theta / (2.0 * np.pi)
            twist = np.sin(moebius_t * np.pi * p.moebius_num_twists)
            displacement = p.moebius_amplitude * twist * np.sin(z_norm * np.pi * p.moebius_freq)

        elif style == LampshadeStyle.HYPER_CUBE_SHADOW:
            rot = z_norm * np.pi * p.hyper_rotation_speed
            w_proj = np.cos(rot) * np.cos(theta * p.hyper_dimension_fold) + np.sin(rot) * np.sin(theta * p.hyper_dimension_fold)
            displacement = p.hyper_amplitude * (np.abs(w_proj) - 0.5) * 2.0

        elif style == LampshadeStyle.CALABI_YAU_PROJECTION:
            n = p.calabi_exponent
            phase = n * (theta + z_norm * p.calabi_phase_drift)
            sin_base = np.abs(np.sin(phase)) + 1e-9
            power = 2.0 / max(n, 1)
            radial = sin_base ** power
            displacement = p.calabi_amplitude * radial * np.cos(theta * n)

        elif style == LampshadeStyle.LISSAJOUS_CAGE:
            cage_disp = 0.0
            for k in range(p.liss_num_curves):
                phase_a = 2.0 * np.pi * k / p.liss_num_curves
                phase_b = np.pi * k / p.liss_num_curves
                lx = np.sin(p.liss_freq_a * z_norm * 2.0 * np.pi + phase_a)
                ly = np.cos(p.liss_freq_b * z_norm * 2.0 * np.pi + phase_b)
                curve_theta = np.arctan2(ly, lx) % (2.0 * np.pi)
                ang_dist = min(abs(theta - curve_theta), 2.0 * np.pi - abs(theta - curve_theta))
                cage_disp += np.exp(-(ang_dist / p.liss_width) ** 2)
            displacement = p.liss_amplitude * cage_disp / p.liss_num_curves

        elif style == LampshadeStyle.SEIFERT_SURFACE_KNOT:
            pk, qk = p.seifert_p, p.seifert_q
            knot_theta = theta * pk + z_norm * 2.0 * np.pi * qk
            knot_r = np.cos(knot_theta * qk / max(pk, 1))
            displacement = p.seifert_amplitude * knot_r * np.sin(z_norm * np.pi)

        elif style == LampshadeStyle.STEREOGRAPHIC_TILING:
            poincare_r = np.tanh(z_norm * p.stereo_compression)
            denom = max(1.0 - poincare_r ** 2, 0.01)
            tile_val = np.sin(theta * p.stereo_tile_p / denom) * np.cos(poincare_r * p.stereo_tile_q * np.pi)
            displacement = p.stereo_amplitude * tile_val * denom

        elif style == LampshadeStyle.PENROSE_QUASICRYSTAL:
            theta_scaled = theta * p.penrose_frequency / (2.0 * np.pi)
            z_scaled = z_norm * p.penrose_frequency
            pen_sum = 0.0
            for k in range(5):
                angle_k = k * 2.0 * np.pi / 5.0
                wave_coord = np.cos(angle_k) * theta_scaled + np.sin(angle_k) * z_scaled
                pen_sum += np.cos(wave_coord * 2.0 * np.pi)
            pen_sum /= 5.0
            displacement = p.penrose_amplitude * np.sign(pen_sum) * np.abs(pen_sum) ** p.penrose_sharpness

        elif style == LampshadeStyle.WEIERSTRASS_MONSTER:
            w_sum = 0.0
            for n_w in range(p.weier_num_terms):
                freq_scale = p.weier_b ** n_w
                amp_scale = (1.0 / p.weier_b) ** (n_w * p.weier_s)
                w_sum += amp_scale * np.cos(freq_scale * np.pi * (theta * p.weier_freq_t / (2.0 * np.pi) + z_norm * p.weier_freq_z))
            displacement = p.weier_amplitude * w_sum

        # MODIFICATION: Apply the passed intensity to the final displacement
        return displacement * intensity

    def _apply_surface_modifiers(self, blended_effect, theta, z_norm, effective_theta):
        """
        Applies surface modifiers to the computed displacement in a fixed order.
        These are post-processing transforms on the displacement value itself.
        """
        p = self.params
        d = blended_effect

        # 1. Domain Warp (recompute with warped coords — approximate by warping displacement)
        if p.enable_smod_domain_warp:
            wt = effective_theta
            wz = z_norm
            for _ in range(p.smod_warp_iterations):
                new_wt = wt + p.smod_warp_amplitude * np.sin(wz * p.smod_warp_freq_z * 2.0 * np.pi + wt * p.smod_warp_freq_t)
                new_wz = wz + p.smod_warp_amplitude * np.cos(wt * p.smod_warp_freq_t * 2.0 + wz * p.smod_warp_freq_z * np.pi)
                wt, wz = new_wt, np.clip(new_wz, 0, 1)
            # Modulate the displacement by the warped-coordinate phase shift
            warp_factor = np.cos((wt - effective_theta) * 3.0) * np.cos((wz - z_norm) * 3.0 * np.pi)
            d *= (0.5 + 0.5 * warp_factor)

        # 2. Kaleidoscope (angular folding of the displacement)
        if p.enable_smod_kaleidoscope:
            sector_angle = 2.0 * np.pi / max(p.smod_kaleidoscope_sectors, 2)
            folded = (effective_theta + p.smod_kaleidoscope_rotation) % sector_angle
            sector_idx = int((effective_theta + p.smod_kaleidoscope_rotation) / sector_angle)
            if sector_idx % 2 == 1:
                folded = sector_angle - folded
            # Use folded angle to modulate displacement symmetrically
            sym_factor = np.cos(folded * np.pi / sector_angle)
            d *= (0.6 + 0.4 * sym_factor)

        # 3. FBM Overlay (additive fractal noise)
        if p.enable_smod_fbm_overlay:
            fbm_val = 0.0
            freq = 1.0
            amp = 1.0
            for i in range(p.smod_fbm_octaves):
                phase_i = i * 1.618  # golden ratio phase offset
                fbm_val += amp * np.sin(effective_theta * freq * p.smod_fbm_base_freq + phase_i) * np.cos(z_norm * freq * p.smod_fbm_base_freq * np.pi)
                freq *= p.smod_fbm_lacunarity
                amp *= p.smod_fbm_gain
            d += p.smod_fbm_amplitude * fbm_val

        # 4. Moiré Interference (additive beat frequency)
        if p.enable_smod_moire:
            p1 = np.sin(effective_theta * p.smod_moire_freq1 + z_norm * p.smod_moire_z_freq1 * 2.0 * np.pi)
            p2 = np.sin(effective_theta * p.smod_moire_freq2 + z_norm * p.smod_moire_z_freq2 * 2.0 * np.pi + p.smod_moire_phase)
            d += p.smod_moire_amplitude * p1 * p2

        # 5. Reaction Warp (additive analytical Gray-Scott approx)
        if p.enable_smod_reaction_warp:
            golden = (1.0 + np.sqrt(5.0)) / 2.0
            u_val = np.cos(effective_theta * p.smod_rd_freq_u) * np.sin(z_norm * np.pi * p.smod_rd_freq_v)
            v_val = np.sin(effective_theta * p.smod_rd_freq_u * golden) * np.cos(z_norm * np.pi * p.smod_rd_freq_v * golden)
            rd_approx = np.tanh(p.smod_rd_sharpness * (u_val * u_val * v_val - v_val))
            d += p.smod_rd_amplitude * rd_approx

        # 6. Signed Power (reshape character)
        if p.enable_smod_signed_power:
            max_range = max(abs(d), 0.01)
            d = np.sign(d) * (np.abs(d / max_range) ** p.smod_power_exponent) * max_range

        # 7. Foldback (wave folding)
        if p.enable_smod_foldback:
            threshold = max(p.smod_fold_threshold, 0.01)
            norm_d = d / threshold
            folded = 4.0 * abs((norm_d / 4.0 + 0.25) % 1.0 - 0.5) - 1.0
            d = folded * threshold * p.smod_fold_gain

        # 8. Quantize (must be last — discretizes)
        if p.enable_smod_quantize:
            levels = max(p.smod_quantize_levels, 2)
            max_range = max(abs(d), 0.01)
            step_size = 2.0 * max_range / levels
            d = np.round(d / step_size) * step_size
    ...
    def _generate_vertex_grid(self):
        ...
        for z_idx in range(p.height_segments + 1):
            ...
            for seg_idx in range(p.radial_segments):
                ...
                # --- Apply Surface Modifiers (post-process displacement) ---
                if p.surface_modifier_chain:
                    blended_effect = self._apply_surface_modifiers(blended_effect, theta, z_norm, effective_theta)

                outer_r = base_r + blended_effect

                # --- Apply Helical Ribbing (additive geometry modifier) ---
                for mod in p.modifier_chain:
                    if mod == "enable_helical_ribbing":
                        for k in range(p.helix_count):
                            helix_theta = effective_theta - z_norm * 2.0 * np.pi * p.helix_pitch + k * 2.0 * np.pi / p.helix_count
                            rib_proximity = np.cos(helix_theta * p.helix_count / 2.0)
                            if rib_proximity > 0:
                                outer_r += p.helix_rib_height * rib_proximity ** p.helix_rib_sharpness
...
class LampshadeApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Surface modifier variables
        self.surface_modifiers_enabled_var = tk.BooleanVar(value=False)
        self.surface_modifier_vars = {name: tk.BooleanVar(value=False) for name in SURFACE_MODIFIER_TOGGLES}
        
        self.smod_quantize_levels_var = tk.IntVar(value=8)
        self.smod_fold_threshold_var = tk.DoubleVar(value=2.5)
        self.smod_fold_gain_var = tk.DoubleVar(value=1.0)
        self.smod_warp_amplitude_var = tk.DoubleVar(value=0.5)
        self.smod_warp_freq_var = tk.IntVar(value=3)
        self.smod_warp_iterations_var = tk.IntVar(value=1)
        self.smod_fbm_octaves_var = tk.IntVar(value=4)
        self.smod_fbm_amplitude_var = tk.DoubleVar(value=3.0)
        self.smod_power_exponent_var = tk.DoubleVar(value=1.0)
        self.smod_moire_freq1_var = tk.IntVar(value=10)
        self.smod_moire_freq2_var = tk.IntVar(value=11)
        self.smod_moire_amplitude_var = tk.DoubleVar(value=3.0)
        self.smod_rd_freq_var = tk.IntVar(value=6)
        self.smod_rd_amplitude_var = tk.DoubleVar(value=3.0)
        self.smod_kaleidoscope_sectors_var = tk.IntVar(value=6)
        ...
        self.helix_count_var = tk.IntVar(value=4)
        self.helix_pitch_var = tk.DoubleVar(value=2.0)
        self.helix_rib_height_var = tk.DoubleVar(value=5.0)
        self.breath_amplitude_var = tk.DoubleVar(value=8.0)
        self.breath_chirp_rate_var = tk.DoubleVar(value=1.5)
    ...
    def _update_all_slider_labels(self):
        ...
        self._update_slider_label(self.helix_count_var, self.helix_count_value_label, is_int=True)
        self._update_slider_label(self.helix_pitch_var, self.helix_pitch_value_label)
        self._update_slider_label(self.helix_rib_height_var, self.helix_rib_height_value_label)
        self._update_slider_label(self.breath_amplitude_var, self.breath_amplitude_value_label)
        self._update_slider_label(self.breath_chirp_rate_var, self.breath_chirp_rate_value_label)

        # Surface modifier slider labels
        self._update_slider_label(self.smod_quantize_levels_var, self.smod_quantize_levels_value_label, is_int=True)
        self._update_slider_label(self.smod_fold_threshold_var, self.smod_fold_threshold_value_label)
        self._update_slider_label(self.smod_fold_gain_var, self.smod_fold_gain_value_label)
        self._update_slider_label(self.smod_warp_amplitude_var, self.smod_warp_amplitude_value_label)
        self._update_slider_label(self.smod_warp_freq_var, self.smod_warp_freq_value_label, is_int=True)
        self._update_slider_label(self.smod_warp_iterations_var, self.smod_warp_iterations_value_label, is_int=True)
        self._update_slider_label(self.smod_fbm_octaves_var, self.smod_fbm_octaves_value_label, is_int=True)
        self._update_slider_label(self.smod_fbm_amplitude_var, self.smod_fbm_amplitude_value_label)
        self._update_slider_label(self.smod_power_exponent_var, self.smod_power_exponent_value_label)
        self._update_slider_label(self.smod_moire_freq1_var, self.smod_moire_freq1_value_label, is_int=True)
        self._update_slider_label(self.smod_moire_freq2_var, self.smod_moire_freq2_value_label, is_int=True)
        self._update_slider_label(self.smod_moire_amplitude_var, self.smod_moire_amplitude_value_label)
        self._update_slider_label(self.smod_rd_freq_var, self.smod_rd_freq_value_label, is_int=True)
        self._update_slider_label(self.smod_rd_amplitude_var, self.smod_rd_amplitude_value_label)
        self._update_slider_label(self.smod_kaleidoscope_sectors_var, self.smod_kaleidoscope_sectors_value_label, is_int=True)
    ...
    def _create_widgets(self):
        ...
        tab_surface_mods = ttk.Frame(right_notebook, padding="10")
        ...
        right_notebook.add(tab_modifiers, text="Shape Modifiers")
        right_notebook.add(tab_surface_mods, text="Surface Modifiers")
        ...
        # --- Populate "Modifiers" Tab (Shape Modifiers) ---
        mod_scroll_canvas = tk.Canvas(tab_modifiers, bg=self.FRAME_BG, highlightthickness=0)
        mod_scrollbar = ttk.Scrollbar(tab_modifiers, orient="vertical", command=mod_scroll_canvas.yview)
        mod_scroll_frame = ttk.Frame(mod_scroll_canvas)
        mod_scroll_frame.bind("<Configure>", lambda e: mod_scroll_canvas.configure(scrollregion=mod_scroll_canvas.bbox("all")))
        mod_scroll_canvas.create_window((0, 0), window=mod_scroll_frame, anchor="nw")
        mod_scroll_canvas.configure(yscrollcommand=mod_scrollbar.set)
        mod_scroll_canvas.pack(side="left", fill="both", expand=True)
        mod_scrollbar.pack(side="right", fill="y")

        ttk.Checkbutton(mod_scroll_frame, text="Enable Shape Modifiers", variable=self.modifiers_enabled_var, command=self._toggle_modifier_controls).pack(anchor="w")
        
        self.modifier_checkboxes = []
        self.modifier_sliders = []

        for name, var in self.modifier_vars.items():
            check = ttk.Checkbutton(mod_scroll_frame, text=name[7:].replace('_', ' ').title(), variable=var)
            check.pack(anchor="w", padx=(10,0))
            self.modifier_checkboxes.append(check)
        
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        ...
        # New shape modifier sliders
        ttk.Separator(mod_scroll_frame, orient="horizontal").pack(fill="x", pady=5)
        ttk.Label(mod_scroll_frame, text="Helical Ribbing").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        self.helix_count_value_label, self.helix_count_slider = self._create_slider_with_label(frame, "Helix Count:", self.helix_count_var, 2, 8, is_int=True)
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        self.helix_pitch_value_label, self.helix_pitch_slider = self._create_slider_with_label(frame, "Helix Pitch:", self.helix_pitch_var, 0.5, 4.0)
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        self.helix_rib_height_value_label, self.helix_rib_height_slider = self._create_slider_with_label(frame, "Rib Height:", self.helix_rib_height_var, 2, 10)
        
        ttk.Label(mod_scroll_frame, text="Breathing Pulse").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        self.breath_amplitude_value_label, self.breath_amplitude_slider = self._create_slider_with_label(frame, "Breath Amp:", self.breath_amplitude_var, 3, 15)
        frame = ttk.Frame(mod_scroll_frame); frame.pack(fill='x', pady=2)
        self.breath_chirp_rate_value_label, self.breath_chirp_rate_slider = self._create_slider_with_label(frame, "Chirp Rate:", self.breath_chirp_rate_var, 0.5, 3.0)
        ...
        self.modifier_sliders.extend([
            ...
            self.twist_angle_slider, self.helix_count_slider, self.helix_pitch_slider,
            self.helix_rib_height_slider, self.breath_amplitude_slider, self.breath_chirp_rate_slider
        ])

        # --- Populate "Surface Modifiers" Tab ---
        smod_scroll_canvas = tk.Canvas(tab_surface_mods, bg=self.FRAME_BG, highlightthickness=0)
        smod_scrollbar = ttk.Scrollbar(tab_surface_mods, orient="vertical", command=smod_scroll_canvas.yview)
        smod_scroll_frame = ttk.Frame(smod_scroll_canvas)
        smod_scroll_frame.bind("<Configure>", lambda e: smod_scroll_canvas.configure(scrollregion=smod_scroll_canvas.bbox("all")))
        smod_scroll_canvas.create_window((0, 0), window=smod_scroll_frame, anchor="nw")
        smod_scroll_canvas.configure(yscrollcommand=smod_scrollbar.set)
        smod_scroll_canvas.pack(side="left", fill="both", expand=True)
        smod_scrollbar.pack(side="right", fill="y")

        ttk.Checkbutton(smod_scroll_frame, text="Enable Surface Modifiers", variable=self.surface_modifiers_enabled_var, command=self._toggle_surface_modifier_controls).pack(anchor="w")

        self.surface_modifier_checkboxes = []
        self.surface_modifier_sliders = []

        for name, var in self.surface_modifier_vars.items():
            label_text = name[13:].replace('_', ' ').title()
            check = ttk.Checkbutton(smod_scroll_frame, text=label_text, variable=var)
            check.pack(anchor="w", padx=(10,0))
            self.surface_modifier_checkboxes.append(check)

        ttk.Separator(smod_scroll_frame, orient="horizontal").pack(fill="x", pady=5)
        ttk.Label(smod_scroll_frame, text="Quantize").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_quantize_levels_value_label, self.smod_quantize_levels_slider = self._create_slider_with_label(frame, "Levels:", self.smod_quantize_levels_var, 2, 20, is_int=True)

        ttk.Label(smod_scroll_frame, text="Foldback").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_fold_threshold_value_label, self.smod_fold_threshold_slider = self._create_slider_with_label(frame, "Threshold:", self.smod_fold_threshold_var, 0.5, 5.0)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_fold_gain_value_label, self.smod_fold_gain_slider = self._create_slider_with_label(frame, "Gain:", self.smod_fold_gain_var, 0.5, 2.0)

        ttk.Label(smod_scroll_frame, text="Domain Warp").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_warp_amplitude_value_label, self.smod_warp_amplitude_slider = self._create_slider_with_label(frame, "Warp Amp:", self.smod_warp_amplitude_var, 0.1, 1.5)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_warp_freq_value_label, self.smod_warp_freq_slider = self._create_slider_with_label(frame, "Warp Freq:", self.smod_warp_freq_var, 1, 6, is_int=True)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_warp_iterations_value_label, self.smod_warp_iterations_slider = self._create_slider_with_label(frame, "Iterations:", self.smod_warp_iterations_var, 1, 3, is_int=True)

        ttk.Label(smod_scroll_frame, text="FBM Overlay").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_fbm_octaves_value_label, self.smod_fbm_octaves_slider = self._create_slider_with_label(frame, "Octaves:", self.smod_fbm_octaves_var, 2, 8, is_int=True)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_fbm_amplitude_value_label, self.smod_fbm_amplitude_slider = self._create_slider_with_label(frame, "FBM Amp:", self.smod_fbm_amplitude_var, 1, 8)

        ttk.Label(smod_scroll_frame, text="Signed Power").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_power_exponent_value_label, self.smod_power_exponent_slider = self._create_slider_with_label(frame, "Exponent:", self.smod_power_exponent_var, 0.2, 5.0)

        ttk.Label(smod_scroll_frame, text="Moiré Interference").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_moire_freq1_value_label, self.smod_moire_freq1_slider = self._create_slider_with_label(frame, "Freq 1:", self.smod_moire_freq1_var, 5, 20, is_int=True)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_moire_freq2_value_label, self.smod_moire_freq2_slider = self._create_slider_with_label(frame, "Freq 2:", self.smod_moire_freq2_var, 6, 21, is_int=True)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_moire_amplitude_value_label, self.smod_moire_amplitude_slider = self._create_slider_with_label(frame, "Moiré Amp:", self.smod_moire_amplitude_var, 1, 8)

        ttk.Label(smod_scroll_frame, text="Reaction Warp").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_rd_freq_value_label, self.smod_rd_freq_slider = self._create_slider_with_label(frame, "RD Freq:", self.smod_rd_freq_var, 3, 12, is_int=True)
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_rd_amplitude_value_label, self.smod_rd_amplitude_slider = self._create_slider_with_label(frame, "RD Amp:", self.smod_rd_amplitude_var, 1, 8)

        ttk.Label(smod_scroll_frame, text="Kaleidoscope").pack(anchor="w", padx=(10,0))
        frame = ttk.Frame(smod_scroll_frame); frame.pack(fill='x', pady=2)
        self.smod_kaleidoscope_sectors_value_label, self.smod_kaleidoscope_sectors_slider = self._create_slider_with_label(frame, "Sectors:", self.smod_kaleidoscope_sectors_var, 3, 12, is_int=True)

        self.surface_modifier_sliders.extend([
            self.smod_quantize_levels_slider, self.smod_fold_threshold_slider, self.smod_fold_gain_slider,
            self.smod_warp_amplitude_slider, self.smod_warp_freq_slider, self.smod_warp_iterations_slider,
            self.smod_fbm_octaves_slider, self.smod_fbm_amplitude_slider, self.smod_power_exponent_slider,
            self.smod_moire_freq1_slider, self.smod_moire_freq2_slider, self.smod_moire_amplitude_slider,
            self.smod_rd_freq_slider, self.smod_rd_amplitude_slider, self.smod_kaleidoscope_sectors_slider
        ])
        ...
        self._toggle_surface_modifier_controls() # Set initial state
    ...
    def _randomize_parameters(self):
        ...
        self.helix_count_var.set(random.randint(2, 8))
        self.helix_pitch_var.set(random.uniform(0.5, 4.0))
        self.helix_rib_height_var.set(random.uniform(2, 10))
        self.breath_amplitude_var.set(random.uniform(3, 15))
        self.breath_chirp_rate_var.set(random.uniform(0.5, 3.0))

        # Randomize surface modifiers
        self.surface_modifiers_enabled_var.set(random.choice([True, False]))
        for var in self.surface_modifier_vars.values():
            var.set(random.choice([True, False]))
        self.smod_quantize_levels_var.set(random.randint(3, 20))
        self.smod_fold_threshold_var.set(random.uniform(0.5, 5.0))
        self.smod_fold_gain_var.set(random.uniform(0.5, 2.0))
        self.smod_warp_amplitude_var.set(random.uniform(0.1, 1.5))
        self.smod_warp_freq_var.set(random.randint(1, 6))
        self.smod_warp_iterations_var.set(random.randint(1, 3))
        self.smod_fbm_octaves_var.set(random.randint(2, 8))
        self.smod_fbm_amplitude_var.set(random.uniform(1, 8))
        self.smod_power_exponent_var.set(random.uniform(0.2, 5.0))
        self.smod_moire_freq1_var.set(random.randint(5, 20))
        self.smod_moire_freq2_var.set(random.randint(6, 21))
        self.smod_moire_amplitude_var.set(random.uniform(1, 8))
        self.smod_rd_freq_var.set(random.randint(3, 12))
        self.smod_rd_amplitude_var.set(random.uniform(1, 8))
        self.smod_kaleidoscope_sectors_var.set(random.randint(3, 12))
            
        self._toggle_blend_controls()
        self._toggle_modifier_controls()
        self._toggle_surface_modifier_controls()
        self._update_all_slider_labels()
    ...
    def _toggle_surface_modifier_controls(self):
        """Enables or disables the surface modifier checkboxes and sliders."""
        state = "normal" if self.surface_modifiers_enabled_var.get() else "disabled"
        for checkbox in self.surface_modifier_checkboxes:
            checkbox.config(state=state)
        for slider in self.surface_modifier_sliders:
            slider.config(state=state)

        if not self.surface_modifiers_enabled_var.get():
            for var in self.surface_modifier_vars.values():
                var.set(False)
    ...
    def _gather_parameters_and_generate(self):
        ...
        if self.modifiers_enabled_var.get():
            ...
            params.helix_count = self.helix_count_var.get()
            params.helix_pitch = self.helix_pitch_var.get()
            params.helix_rib_height = self.helix_rib_height_var.get()
            params.breath_amplitude = self.breath_amplitude_var.get()
            params.breath_chirp_rate = self.breath_chirp_rate_var.get()
        else:
        ...
        # --- Surface Modifiers ---
        if self.surface_modifiers_enabled_var.get():
            for name, var in self.surface_modifier_vars.items():
                setattr(params, name, var.get())
            params.smod_quantize_levels = self.smod_quantize_levels_var.get()
            params.smod_fold_threshold = self.smod_fold_threshold_var.get()
            params.smod_fold_gain = self.smod_fold_gain_var.get()
            params.smod_warp_amplitude = self.smod_warp_amplitude_var.get()
            params.smod_warp_freq_t = self.smod_warp_freq_var.get()
            params.smod_warp_freq_z = self.smod_warp_freq_var.get()
            params.smod_warp_iterations = self.smod_warp_iterations_var.get()
            params.smod_fbm_octaves = self.smod_fbm_octaves_var.get()
            params.smod_fbm_amplitude = self.smod_fbm_amplitude_var.get()
            params.smod_power_exponent = self.smod_power_exponent_var.get()
            params.smod_moire_freq1 = self.smod_moire_freq1_var.get()
            params.smod_moire_freq2 = self.smod_moire_freq2_var.get()
            params.smod_moire_amplitude = self.smod_moire_amplitude_var.get()
            params.smod_rd_freq_u = self.smod_rd_freq_var.get()
            params.smod_rd_freq_v = self.smod_rd_freq_var.get()
            params.smod_rd_amplitude = self.smod_rd_amplitude_var.get()
            params.smod_kaleidoscope_sectors = self.smod_kaleidoscope_sectors_var.get()
        else:
            for name in self.surface_modifier_vars:
                setattr(params, name, False)
